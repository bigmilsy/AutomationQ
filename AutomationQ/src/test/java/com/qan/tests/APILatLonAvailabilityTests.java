...
public class APILatLonAvailabilityTests {
    ...
    public void checkAvailability() {
        //Setup API URL for the test
        String testUrl = Helpers.buildCurrentURLLatLon(BaseUrl, testData.data.get("checkAvailability").lat, testData.data.get("checkAvailability").lon);
        //Check to ensure that we see the expected response codel
        given().when().get(testUrl).then().assertThat().statusCode(expectedStatusCode);
    }    
